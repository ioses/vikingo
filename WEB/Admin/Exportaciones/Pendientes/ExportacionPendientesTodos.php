...
$sqlPendientes="SELECT identificador_paciente.Id_Hospital, AES_DECRYPT (identificador_paciente.NHC,'$claveNHC') AS NHC, tabla_patologica.Estado AS ESTADO, 
	      		tratamiento.B_Tto_Ady AS ADY, cirugia.ID AS IDCir FROM identificador_paciente INNER JOIN tabla_patologica 
	      		ON identificador_paciente.ID=tabla_patologica.Id_Paciente INNER JOIN tratamiento 
	      		ON identificador_paciente.ID=tratamiento.Id_Paciente  
	      		INNER JOIN cirugia ON identificador_paciente.ID=cirugia.Id_Paciente ORDER BY identificador_paciente.Id_Hospital ASC";  
...
header("Content-Disposition: attachment; filename=Pendientes_Completo.xls"); 
...
$sqlSiCirugia="SELECT B_Cirugia FROM cirugia WHERE ID='$IdCirugia'";
						...
						$rowCir=mysqli_fetch_array(mysqli_query($conexion,$sqlSiCirugia))
				...
				if ($Cirugia==1){
					...
					$rowFechaAlta=mysqli_fetch_array(mysqli_query($conexion,$sqlFechaAlta))
       						or die('Error: ' . mysqli_error());
       						
       				$FechaAlta=$rowFechaAlta[0];		
				}
