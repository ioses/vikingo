...
$(document).ready(function() {
    ...
    if ($("#Tecnicas_Cirugia").val() == "7" || $("#Tecnicas_Cirugia").val() == "8" || $("#Tipo_Reseccion").val == "3")
    {
       
        $("#B_Recidiva_Si").popover({
                title: "Error",
                content: "No se puede optar por está opción ya que la técnica quirúrgica es tal tal tal...",
                trigger: "hover",
                delay: { show: 20, hide: 1000 }
        });
        

        //$("#B_Recidiva_Si").attr("disabled", "disabled");
    }
    
    $("#B_Recidiva_Si").mouseleave(function()
    {
        //$(this).popover('hide');
        if ($("#Tecnicas_Cirugia").val() == "7" || $("#Tecnicas_Cirugia").val() == "8" || $("#Tipo_Reseccion").val == "3")
        {
      
            $("#B_Recidiva_Si").prop('checked',false);
            $("#B_Recidiva_No").prop('checked',true);
            HabilitaRecidiva();
        }
    });
    /*alert($("#Tecnicas_Cirugia").val());
   alert($("#Tipo_Reseccion").val());*/
    
    /*
     * 
     * 
     * Metástasis inicial No == 0;
     *                    Si == 1;
     * 
     * Metástatis hepática Si = 1;
     *                     No = 2;
     *                     No valoradas = 3;
     * 
     * Implantes ováricos ConCheck = on
     *                     SinCheck = (sale vacío)
     *  
     */
    if ($("#MetastInicial").val() == "1" || $("#Tipo_Metast_Hepaticas").val() == "1" || $("#Imp_Ovaricos").val == "on")
    { 
        
        $("#B_Metastasis_Si").popover({
                title: "Error",
                content: "No se puede optar por está opción ya hay metástasis...",
                trigger: "hover",
                delay: { show: 20, hide: 1000 }
        });
        
        //$("#B_Metastasis_Si").attr("disabled", "disabled");
    }
    
    $("#B_Metastasis_Si").mouseleave(function()
    {
        //$(this).popover('hide');
        if ($("#MetastInicial").val() == "1" || $("#Tipo_Metast_Hepaticas").val() == "1" || $("#Imp_Ovaricos").val == "on")
        { 
            $("#B_Metastasis_Si").prop('checked',false);
            $("#B_Metastasis_No").prop('checked',true);
            HabilitaMetastasis();
        }
    });
    
   //alert($("#MetastInicial").val());
   //alert($("#Tipo_Metast_Hepaticas").val());
    //alert($("#Imp_Ovaricos").val());
