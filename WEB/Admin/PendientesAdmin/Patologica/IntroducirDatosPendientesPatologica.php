...
$sqlId="SELECT ID FROM identificador_paciente WHERE NHC=AES_ENCRYPT('$NHC','$claveNHC') AND Id_Hospital = '$idHospital'";
  ...
  $sqlPatologicaSi="UPDATE tabla_patologica SET Estado= '1' WHERE Id_Paciente='$Id_Paciente'";
	...
	$sqlIdCirugia="SELECT ID FROM cirugia WHERE Id_Paciente='$Id_Paciente'";	
	...
	$sqlExeresisMeso="SELECT Id_Exeresis_Meso FROM tabla_cirugia WHERE Id_Cirugia='$IdCirugia'";			
   
   	$res=mysqli_query($conexion,$sqlExeresisMeso)
            or die('Error: ' . mysqli_error());	
	
	$row=mysqli_fetch_array($res);
	$Exeresis_Meso=intval($row[0]);
